-- Rollback for 0009_native_colourway.sql
-- wrangler d1 migrations only runs forward, so apply this by hand:
--   npm run d1:rollback:native-colourway:local
--
-- Rebuilds fan_profiles back to 0008's CHECK constraint (the 12 real ids
-- only, no 'native'). Any fan currently opted into 'native' would violate
-- the narrowed CHECK, so those rows are reset to NULL (falls back to their
-- deterministically assigned colourway on next read — see sprites.ts's
-- assignColourway) rather than the rebuild failing outright. Same
-- id-preservation / index-recreation / FK-cascade-guard pattern as every
...
SELECT
  ...
  collection_count, created_at, updated_at, last_seen_at, handle_changed_at,
  ep, stage, hatched_at, sprite_egg, sprite_grub, sprite_pupa, sprite_adult,
  CASE colourway WHEN 'native' THEN NULL ELSE colourway END,
  override_sprite
FROM fan_profiles;
...
DELETE FROM d1_migrations WHERE name = '0009_native_colourway.sql';
